...
# define ERR_NOSUCHNICK(user, name) PROMPT(" 401 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :No such nick/channel")
// Utilisé pour indiquer que le pseudonyme passé en paramètre à la commande n'est pas actuellement utilisé.

# define ERR_NOSUCHSERVER(user, name) PROMPT(" 402 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :No such server")
// Utilisé pour indiquer que le nom du serveur donné n'existe pas actuellement.

# define ERR_NOSUCHCHANNEL(user, name) PROMPT(" 403 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :No such channel")
// Utilisé pour indiquer que le nom de canal donné est invalide.

# define ERR_CANNOTSENDTOCHAN(user, name) PROMPT(" 404 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Cannot send to channel")
...
# define ERR_TOOMANYCHANNELS(user, name) PROMPT(" 405 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :You have joined too many channels")
...
# define ERR_WASNOSUCHNICK(user, name) PROMPT(" 406 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :There was no such nickname")
// Renvoyé par WHOWAS pour indiquer qu'il n'y a pas d'information dans l'historique concernant ce pseudonyme.

# define ERR_TOOMANYTARGETS(user, name) PROMPT(" 407 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Duplicate recipients. No message delivered")
...
# define ERR_NOTOPLEVEL(user, domain) PROMPT(" 413 ", user) + std::string(" ") + std::string(" ") +  + std::string(" <masque> :No toplevel domain specified" //
// Domaine principal non spécifié.

# define ERR_WILDTOPLEVEL(user, domain) PROMPT(" 414 ", user) + std::string(" ") + std::string(" ") +  + std::string(" <masque> :Wildcard in toplevel domain" //
...
# define ERR_NOADMININFO(user, name) PROMPT(" 423 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :No administrative info available")
...
# define ERR_ERRONEUSNICKNAME(user, name) PROMPT(" 432 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Erroneus nickname")
...
# define ERR_NICKNAMEINUSE(user, name) PROMPT(" 433 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Nickname is already in use")
...
# define ERR_KEYSET(user, name) PROMPT(" 467 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Channel key already set")
// Clé de canal déjà définie.

# define ERR_CHANNELISFULL(user, name) PROMPT(" 471 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Cannot join channel (+l)")
// Impossible de joindre le canal (+l)

# define ERR_UNKNOWNMODE(user, arg) PROMPT(" 472 ", user) + std::string(" ") + std::string(" ") + arg + std::string(" :is unknown mode char to me")
// Mode inconnu.

# define ERR_INVITEONLYCHAN(user, name) PROMPT(" 473 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Cannot join channel (+i)")
// Impossible de joindre le canal (+i).

# define ERR_BANNEDFROMCHAN(user, name) PROMPT(" 474 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Cannot join channel (+b)")
// Impossible de joindre le canal (+b).

# define ERR_BADCHANNELKEY(user, name) PROMPT(" 475 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :Cannot join channel (+k)")
...
# define ERR_CHANOPRIVSNEEDED(user, name) PROMPT(" 482 ", user) + std::string(" ") + std::string(" ") + name + std::string(" :You're not channel operator")
