...
# define ERR_UNKNOWNERROR(user, subcommand, info) PROMPT(" 400 ", user) + std::string(" ") + (subcommand.empty() ? "" : " ") + subcommand) + std::string(" :") + info
// Indicates that the given command/subcommand could not be processed. <subcommand> may repeat for more specific subcommands.

# define ERR_NOSUCHNICK(user, name) PROMPT(" 401 ", user) + std::string(" ") + name + std::string(" :No such nick/channel")
// Utilisé pour indiquer que le pseudonyme passé en paramètre à la commande n'est pas actuellement utilisé.

# define ERR_NOSUCHSERVER(user, name) PROMPT(" 402 ", user) + std::string(" ") + name + std::string(" :No such server")
// Utilisé pour indiquer que le nom du serveur donné n'existe pas actuellement.

# define ERR_NOSUCHCHANNEL(user, name) PROMPT(" 403 ", user) + std::string(" ") + name + std::string(" :No such channel")
// Utilisé pour indiquer que le nom de canal donné est invalide.

# define ERR_CANNOTSENDTOCHAN(user, name) PROMPT(" 404 ", user) + std::string(" ") + name + std::string(" :Cannot send to channel")
...
# define ERR_TOOMANYCHANNELS(user, name) PROMPT(" 405 ", user) + std::string(" ") + name + std::string(" :You have joined too many channels")
...
# define ERR_WASNOSUCHNICK(user, name) PROMPT(" 406 ", user) + std::string(" ") + name + std::string(" :There was no such nickname")
// Renvoyé par WHOWAS pour indiquer qu'il n'y a pas d'information dans l'historique concernant ce pseudonyme.

# define ERR_TOOMANYTARGETS(user, name) PROMPT(" 407 ", user) + std::string(" ") + name + std::string(" :Duplicate recipients. No message delivered")
...
# define ERR_NOORIGIN(user) PROMPT(" 409 ", user) + std::string(" ") + std::string(" :No origin specified")
// Message PING ou PONG sans le paramètre origine qui est obligatoire puisque ces commandes doivent marcher sans préfixe.

# define ERR_NORECIPIENT(user, command) PROMPT(" 411 ", user) + std::string(" ") + std::string(" :No recipient given (<") + command + std::string(">)")
// Pas de destinataire.

# define ERR_NOTEXTTOSEND(user) PROMPT(" 412 ", user) + std::string(" ") + std::string(" :No text to send")
// Pas de texte à envoyer.

# define ERR_NOTOPLEVEL(user, domain) PROMPT(" 413 ", user) + std::string(" ") +  + std::string(" <masque> :No toplevel domain specified" //
// Domaine principal non spécifié.

# define ERR_WILDTOPLEVEL(user, domain) PROMPT(" 414 ", user) + std::string(" ") +  + std::string(" <masque> :Wildcard in toplevel domain" //
...
# define ERR_NOADMININFO(user, name) PROMPT(" 423 ", user) + std::string(" ") + name + std::string(" :No administrative info available")
...
# define ERR_NONICKNAMEGIVEN(user) PROMPT(" 431 ", user) + std::string(" ") + std::string(" :No nickname given")
// Renvoyé quand un paramètre pseudonyme attendu pour une commande n'est pas fourni.

# define ERR_ERRONEUSNICKNAME(user, name) PROMPT(" 432 ", user) + std::string(" ") + name + std::string(" :Erroneus nickname")
...
# define ERR_NICKNAMEINUSE(user, name) PROMPT(" 433 ", user) + std::string(" ") + name + std::string(" :Nickname is already in use")
// Renvoyé quand le traitement d'un message NICK résulte en une tentative de changer de pseudonyme en un déjà existant.

# define ERR_NICKCOLLISION(user, name) PROMPT(" 436 ", user) + std::string(" ") + name + std::string(" :Nickname collision KILL")
...
# define ERR_USERNOTINCHANNEL(user, nickname, channel) PROMPT(" 441 ", user) + std::string(" ") + nickname + std::string(" ") + channel + std::string(" :They aren't on that channel")
// Renvoyé par un serveur pour indiquer que l'utilisateur donné n'est pas dans le canal spécifié.

# define ERR_NOTONCHANNEL(user, name) PROMPT(" 442 ", user) + std::string(" ") + name + std::string(" :You're not on that channel")
...
# define ERR_ALREADYREGISTRED(user) PROMPT(" 462 ", user) + std::string(" ") + std::string(" :You may not reregister")
...
# define ERR_NOPERMFORHOST(user) PROMPT(" 463 ", user) + std::string(" ") + std::string(" :Your host isn't among the privileged")
// Renvoyé à un client qui essaie de s'enregistrer sur un serveur qui n'accepte pas les connexions depuis cet hôte.

# define ERR_PASSWDMISMATCH(user) PROMPT(" 464 ", user) + std::string(" ") + std::string(" :Password incorrect")
...
# define ERR_YOUREBANNEDCREEP(user) PROMPT(" 465 ", user) + std::string(" ") + std::string(" :You are banned from this server")
...
# define ERR_KEYSET(user, name) PROMPT(" 467 ", user) + std::string(" ") + name + std::string(" :Channel key already set")
// Clé de canal déjà définie.

# define ERR_CHANNELISFULL(user, name) PROMPT(" 471 ", user) + std::string(" ") + name + std::string(" :Cannot join channel (+l)")
// Impossible de joindre le canal (+l)

# define ERR_UNKNOWNMODE(user, arg) PROMPT(" 472 ", user) + std::string(" ") + arg + std::string(" :is unknown mode char to me")
// Mode inconnu.

# define ERR_INVITEONLYCHAN(user, name) PROMPT(" 473 ", user) + std::string(" ") + name + std::string(" :Cannot join channel (+i)")
// Impossible de joindre le canal (+i).

# define ERR_BANNEDFROMCHAN(user, name) PROMPT(" 474 ", user) + std::string(" ") + name + std::string(" :Cannot join channel (+b)")
// Impossible de joindre le canal (+b).

# define ERR_BADCHANNELKEY(user, name) PROMPT(" 475 ", user) + std::string(" ") + name + std::string(" :Cannot join channel (+k)")
// Impossible de joindre le canal (+k).

# define ERR_NOPRIVILEGES(user) PROMPT(" 481 ", user) + std::string(" ") + std::string(" :Permission Denied- You're not an IRC operator")
// Toute commande qui requiert le privilège d'opérateur pour opérer doit retourner cette erreur pour indiquer son échec.

# define ERR_CHANOPRIVSNEEDED(user, name) PROMPT(" 482 ", user) + std::string(" ") + name + std::string(" :You're not channel operator")
...
# define ERR_CANTKILLSERVER(user) PROMPT(" 483 ", user) + std::string(" ") + std::string(" :You cant kill a server!")
...
# define ERR_NOOPERHOST(user) PROMPT(" 491 ", user) + std::string(" ") + std::string(" :No O-lines for your host")
...
# define ERR_UMODEUNKNOWNFLAG(user) PROMPT(" 501 ", user) + std::string(" ") + std::string(" :Unknown MODE flag")
...
# define ERR_USERSDONTMATCH(user) PROMPT(" 502 ", user) + std::string(" ") + std::string(" :Cant change mode for other users")
