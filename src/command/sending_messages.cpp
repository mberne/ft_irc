	...
	else
	{
		...
		for (size_t i = 0; i < target.size(); i++)
		{
			if (target[i][0] != '#')
			{
				...
				{
					serv->getClient(target[i])->addToOutputBuffer(sender->getPrefix() + " " + cmd[0] + " " + target[i] + " :" + cmd[2]);
			...
			else
			{
				...
				{
					serv->getChannel(target[i])->sendToClients(sender->getPrefix() + " " + cmd[0] + " " + target[i] + " :" + cmd[2], sender);
					serv->addLog("from: " + sender->getPrefix() + " to: " + target[i] + "\n" + cmd[2], LOG_MESSAGE);
				}
				else
	...
	if (cmd.size() > 2)
	{
		...
		for (size_t i = 0; i < target.size(); i++)
		{
			if (!sender)
			{
				...
				serv->getClient(target[i])->addToOutputBuffer(SERV_NAME + " " + cmd[0] + " " + target[i] + " :" + cmd[2]);
			}
